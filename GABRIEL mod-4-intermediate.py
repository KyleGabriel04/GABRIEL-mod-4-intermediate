...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    # Check if the input letter is a space or underscore
    if letter == " " or letter == "_":
        return " "

    letter = letter.upper()

    # Check if the shift value is a digit
    ...
    if ord(letter) + shift <= ord("Z"):
        ...
        shifted_letter = chr(ord("A") + shift)

    return shifted_letter

letter = str(input("Select a Letter: "))
shift = input("Select a Number or underscore (_): ")
...
def caesar_cipher(message, shift):
    ...
    for char in message:
        if char == " ":
            ...
            shift_character = (ord(char) + shift - ord("A")) % 26 + ord("A")
